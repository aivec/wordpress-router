 ...
 *
 * This router operates via POST key value pairs. This version of the router is necessary
 * for websites that use the traditional `/?p=123` post ID permalink structure. In this instance,
 * it's not possible to use normal route resolving.
 */
class PostIdRouter {
            ...
            if (isset($_REQUEST['route'])) {
                ...
                if ($_REQUEST['route'] === $route && $n === $namespace) {
                    ...
                    foreach ($middlewares as $middleware) {
                        call_user_func($middleware, $payload);
                    }
                    $res = call_user_func($callable, $payload);
                    if (empty($res)) {
                        die(0);
                    }
                    die($res);
